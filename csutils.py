...
def create_table_stmt(table_name,config_file):
    """
    Creates as SQL statement for a given table using the cofiguration file that contains all the column configuration for it
    
    Inputs:
        table_name [String] : table name that needs to be created. It must have corresponding section in the configuration file
        config_file[String] : file path to configuration file where table configruation is stored
    """
    # Read table configuration
    cols = config.get_table_config(table_name, filename = config_file)
    
    # Prepare SQL statemtn to Create table
    stmt = "CREATE TABLE {} ( {} );"
    col_stmt = []
    
    for key, item in cols.items():
        col_stmt.append(f'"{key}" {item}')
    
    stmt = stmt.format(table_name,",".join(col_stmt))
    
    return stmt

def create_tbl_comments(table_name, config_file, dbconn):
    """
    Creates table comments in the database for a given table. reads the comments sections in the config file provided
    
    Inputs:
        table_name[String] : table name for which comments needs to be updated
        config_file[ tring] : path to the config file.ini where table comments are listed 
        dbcomm[SQLAlchemy connection] : SQL Alchmey db connection object
    """

    # Read table comments as dictionary
    
    col_comments = config.get_table_comments(table_name,config_file)

    stmt = "COMMENT On COLUMN {}.{} is '{}' "

    for col, comment in col_comments.items():
        # If the comment is for the whole table
        if col == "table":
            stmt_tbl = f"comment on table {table_name} is '{comment}'"

            dbconn.execute(text(stmt_tbl).execution_options(autocommit=True))

        else:
            # of if the comments is for a certain column
            dbconn.execute(text(stmt.format(table_name,col,comment)).execution_options(autocommit=True))





##### Climate Watch functions ###############

def dl_climatewatch_emissions(em_url, tbl_name):
    """
    Download GHG emissions data from Climate watch website using their API.
    Structure the data accordingly to the table name

    Input : 
        em_url[String] : Base url form GHG emissins API
        tbl_name[String] : Database table name to use column names
    Returns:
        [dataframe]: Dataframe for GHG emissions for all countries, datasources and gas types
    """
    ...
    # List to collect page response converted into dataframe
    df_ems = []
    ...
    while has_next:
        ...
        try:
            resp = requests.get(em_url, params= parameters)

            if resp.status_code !=200:
                print(f"Error while getting data from url : {em_url}")
                break

            # If there is successfull response from request
            else:
                
                # If it is the first page then find the last page number
                if parameters.get("page") == 1:
                    # link for the lastp page url
                    lurl = resp.headers.get("Link").split(";")[0][1:-1]
                    
                    # Get the last page number from respone headers
                    last_page = int(parse_qs(urlparse(lurl).query)["page"][0])
                ...
                df_em = pd.json_normalize(data["data"], 
                                        record_path=["emissions"], 
                                        meta=["iso_code3","data_source","sector", "gas"]
                                        )
                # add dataframe to list
                df_ems.append(df_em)
                
                # increment the page number if last page is still not reached
                if last_page > parameters.get("page"):
                    parameters["page"] += 1
                    
                    # add random sleep to avoid throttling the server
            ...
            print(f"Error happened while connecting to the URL: {em_url}")

    print(f" pages collected : {len(df_ems)} , total pages : {last_page}")

    # Convert from list into a single dataframe
    df_emt = pd.concat(df_ems)

    cols = config.get_table_config(tbl_name)
    cols = list(cols.keys())

    # Select required columns from dataframe to be stored in table
    df_emt = df_emt[cols]
...
def sunburst_tree_orig(pc, df_data,key_col,val_col):
    """
    Takes in tree structure from "treelib" and convert it into the format as needed by E-Charts sunburst chart.
    ...
    if isinstance(pc, dict):
        for key, value in pc.items():
            if key == "children":
                ...
                rets = { "name": key,
                        "children": sunburst_tree(value, df_data,key_col,val_col)
                       }
                if df_data[df_data[key_col]==key][val_col].values.size>0:
                    rets["value"] = df_data[df_data[key_col]==key][val_col].values[0]
                return rets
    ...
    if isinstance(pc, str):
            ...
            if df_data[df_data[key_col]==pc][val_col].values.size>0:
                rets["value"] = df_data[df_data[key_col]==pc][val_col].values[0]
            return rets
    

def sbtree_update(sb_tree,item_color= None):
    """
    Function to update tree structure and add attributes for each node 
    
    Input: 
        sb_tree (dict) : E-charts Sun burst dictionary data, which needs to be summed up bottom up
        item_color ( dict) : Dictionary to add item background color for each node
    Output:
        rest ( dict) : Updated e-charts sun burst dictionary
    """
    
    if isinstance(sb_tree, list):

        return [sbtree_update(item, item_color) for item in sb_tree]

    if isinstance(sb_tree, dict):
        

        if sb_tree["name"] in item_color.keys():
            rets = sb_tree
            sb_tree["itemStyle"] = { "color": item_color.get(rets["name"])}
        
        
        # if it is the leaf node
        if not "children" in sb_tree.keys():
            
            # Add color style item to the node
            rets = sb_tree
            rets["itemStyle"] = { "color": item_color.get(rets["name"])}
            ...
            rets["children"] = sbtree_update(sb_tree["children"], item_color)
            # rets = { "name": sb_tree["name"],
            #         "children": sbtree_update(sb_tree["children"])
            #        }
             
            #rets["value"] = sum( [ sbtree_update_vals(item)["value"] for item in sb_tree["children"] ] )
...
def get_nasa_temp_anom(region = "GLB", base_url = None, skip_rows = 2, cols =  ["year","month","anom"]):
    """
    Downloads Temperature anomalies data from NASA GISS temp site using CSV file.
    Note, it is very specific to the type of files on https://data.giss.nasa.gov/gistemp/#tabledata
    If the format of the files changes, it would need a change as well in the code
    
    Inputs: 
        region(String): Region doe which temp anomalies data to be downloaded. GLB, NH and SH are valid entities
        base_url (String): In case there is a new base url for these files
        skip_rows (int) : Number of lines to skip in the csv file
        cols = (list): List of names to give for the intial 3 columns
    Output:
        Dataframe with monthly temperature anomalies with region and date added as additional columns
    
    """
    if base_url is None:
        base_url = "https://data.giss.nasa.gov/gistemp/tabledata_v4/{}.Ts+dSST.csv"
    
    # Download data from NAS website
    try:
        url = base_url.format("GLB")
        resp = requests.get(url)

        if resp.status_code != 200:
            print(f"Could not find file : {url} on the server.")

        else:
            data = resp.text
    except:
        print("There was error while downloading file from server. Please check link manually")
        print(f"URL : (url)")

    
        
    # Read csv in pandas dataframe
    df_tmp = pd.read_csv(io.StringIO(data), skiprows=skip_rows, index_col=False, header=None)

    # Keep only monthy information. Assumes that 1st 13 columns contains the monthly values with 1st column being year
    df_tmp = df_tmp.iloc[:,:13].set_index(0)
    df_tmp.index.name = "year"

    # rearrange the data to have year and month as columns
    df_tmp = df_tmp.stack()
    df_tmp = df_tmp.reset_index(level=[0,1])

    # give names to the columns
    df_tmp.columns = cols

    # Remove strings *** from the column and make the datatype float
    df_tmp = df_tmp[df_tmp["anom"] != "***"]
    df_tmp["anom"] = df_tmp.anom.astype(float)

    # Add date column
    df_tmp["date"] = df_tmp.apply(lambda row : datetime.date(int(row.year),int(row.month),1), axis=1)
...
def dl_noaa_gml_data(site_code, base_url,verbose= False):
    """
    Download NOAA GML finder data for a given site code using base url for the respective indicator.
    
    NOTE: For now it is only using site_code as variable 
    Assumptions:
        Only sitecode needs to be changes for a given GHG indicator
        
    
    Input: 
        site_code(string): site_code of NOAA measurement site
        base_url (string): url string for the data file with placeholder to replace site_code
    """
    
    url= base_url.format(site_code)
    
    try:
        response = requests.get(url)
        
        if response.status_code != 200:
            if verbose: print("The file does not exist")
            return None
        
        data = response.text
    
    except:
        print("Error Reading the file. Check if url is correct")
        print(f"URL : {url}")
        return
    
    skip_lines = int(pd.read_table(io.StringIO(data), nrows=1).columns[0].split(":")[-1].strip())
    
    if skip_lines is None:
        print("data file does not follow the standard, please check the content manually")
        return
    
    df = pd.read_table(io.StringIO(data), skiprows= skip_lines-1, delim_whitespace =True)
    cols = df.columns.copy()
    df = df.dropna(axis=1, how="all")
    df.columns = cols[len(cols) - len(df.columns):]
